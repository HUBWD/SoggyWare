...
local a, b = pcall(function()
	...
	local function getTeamMates()
		local team = {}
		for i, v in next, plr.PlayerGui:GetChildren() do
			if v.Name == "NameGui" then
				table.insert(team, v.Adornee.Parent.Parent.Name)
			end
		end
		return team
	end
	local function getCharacter()
		local mag, dist = 0, math.huge
		local localCharacter = nil
		for i, v in next, workspace.Characters:GetChildren() do
			if (v.Body.Head.Position - camera.CFrame.p).magnitude <= dist and v and v.Health.Value >= 1 then
				dist = (v.Body.Head.Position - camera.CFrame.p).magnitude
				localCharacter = v
			end
		end
	...
	local function getClosestPlr()
		...
		for i, v in next, workspace.Characters:GetChildren() do
			if v and v.Health.Value >= 1 and v.Body.Head and not table.find(teamMates, v.Name) then
				local pos, vis = camera:WorldToScreenPoint(v.Body.Head.Position)
				local character = getCharacter()
				if vis and (character.Body.Head.Position - v.Body.Head.Position).Magnitude <= 1000 then
	...
	local function AddESP(v)
		task.wait(.05)
		local team = getTeamMates()
		if not table.find(team, v.Name) then
			ESP:Add(v, {
				Name = v.Name,
				PrimaryPart = v:WaitForChild("Root")
			})
		end
	end
	pcall(function()
		workspace.Characters.ChildAdded:Connect(AddESP)
		for i, v in next, workspace.Characters:GetChildren() do
			AddESP(v)
		end
	end)
	...
	local win = SolarisLib:New({
		Name = "Bad Business",
		FolderToSave = "BB_SGWR"
	})
	local tab = win:Tab("Combat")
	local sec = tab:Section("Combat")
	sec:Toggle("Aim Bot", false, "Aim Bot", function(t)
		getgenv().AimBot = t
		while getgenv().AimBot == true do
			task.wait()
			if game:GetService("UserInputService"):IsMouseButtonPressed(Enum.UserInputType.MouseButton2) then
				pcall(function()
					local pos, vis = camera:WorldToScreenPoint(getClosestPlr().Body.Head.Position)
					mousemoverel((pos.X - mouse.X) * 0.5, (pos.Y - mouse.Y) * 0.5)
				end)
			end
		end
	end)
	local sec = tab:Section("ESP")
	sec:Toggle("ESP", false, "ESP", function(t)
		ESP:Toggle(t)
	end)
	sec:Toggle("Tracers", false, "Tracers", function(t)
		ESP.Tracers = t
	end)
	sec:Toggle("Boxes", false, "Boxes", function(t)
		ESP.Boxes = t
	end)
	sec:Toggle("Names", false, "Names", function(t)
		ESP.Names = t
	end)
end)
