...
pcall(function()
	...
	local function GetClosestPlayer()
		...
		for i, v in next, game:GetService("Players"):GetPlayers() do
			if v then
				if v.Character and v.Character:FindFirstChild("Humanoid") and v.Character:FindFirstChild("Humanoid").Health >= 1 and not v.Character:FindFirstChildOfClass("BillboardGui") then
					local pos, vis = cam:WorldToScreenPoint(v.Character.Head.Position)
					if vis and (game.Players.LocalPlayer.Character.Head.Position - v.Character.Head.Position).Magnitude <= 1000 then
	...
	local function AddESP(v, colour)
        pcall(function()
            ESP:Add(v, {
                Name = v.Name,
                PrimaryPart = v.Character:FindFirstChild("HumanoidRootPart"),
                Color = colour
            })
        end)
	end
			...
			if getgenv()["esp"] then
				for i, v in next, game:GetService("Players"):GetPlayers() do
					if v and v ~= game.Players.LocalPlayer then
						if v.Character and v.Character:FindFirstChild("Humanoid") and v.Character:FindFirstChild("Humanoid").Health >= 1 then
							AddESP(v, not v.Character:FindFirstChildOfClass("BillboardGui") and Color3.fromRGB(255, 0, 0) or Color3.fromRGB(0, 255, 0))
						end
					end
				end
			end
	...
	local main = library:Load{
		...
		Folder = "aimblox soggyware" -- config folder name
	}
	...
	section:Toggle{
		...
		Callback  = function(t)
			getgenv()["Aim Bot"] = t
			while getgenv()["Aim Bot"] == true do
				task.wait()
				if game:GetService("UserInputService"):IsMouseButtonPressed(Enum.UserInputType.MouseButton2) then
					pcall(function()
						local pos, vis = cam:WorldToScreenPoint(GetClosestPlayer().Character.Head.CFrame.p)
	...
	section:Toggle{
		...
		Flag = "Trigger Bot",
		Callback  = function(t)
			getgenv()["Trigger Bot"] = t
			while getgenv()["Trigger Bot"] == true do
				task.wait()
                pcall(TriggerBot)
			end
		end
